...
async def list_installed_docsets(ctx: Context) -> DocsetResults:
    ...
    try:
        ...
        if base_url is None:
            return DocsetResults(error="Failed to connect to Dash API Server. Please ensure Dash is running and the API server is enabled (Settings > Integration, or run open -b com.kapeli.dash-setapp, followed by defaults write com.kapeli.dash-setapp DHAPIServerEnabled YES).")
...
async def search_documentation(
    ...
    try:
        base_url = await working_api_base_url(ctx)
        if base_url is None:
            return SearchResults(error="Failed to connect to Dash API Server. Please ensure Dash is running and the API server is enabled (Settings > Integration, or run open -b com.kapeli.dash-setapp, followed by defaults write com.kapeli.dash-setapp DHAPIServerEnabled YES).")
        ...
        if e.response.status_code == 400:
            ...
            if "Docset with identifier" in error_text and "not found" in error_text:
                ...
                return SearchResults(error=f"Bad request: {error_text}. Please ensure Dash is running and the API server is enabled (Settings > Integration, or run open -b com.kapeli.dash-setapp, followed by defaults write com.kapeli.dash-setapp DHAPIServerEnabled YES).")
            ...
            if "API access blocked due to Dash trial expiration" in error_text:
                ...
                return SearchResults(error=f"Forbidden: {error_text}. Please ensure Dash is running and the API server is enabled (Settings > Integration, or run open -b com.kapeli.dash-setapp, followed by defaults write com.kapeli.dash-setapp DHAPIServerEnabled YES).")
        await ctx.error(f"HTTP error: {e}")
        return SearchResults(error=f"HTTP error: {e}. Please ensure Dash is running and the API server is enabled (Settings > Integration, or run open -b com.kapeli.dash-setapp, followed by defaults write com.kapeli.dash-setapp DHAPIServerEnabled YES).")
    except Exception as e:
        await ctx.error(f"Search failed: {e}")
        return SearchResults(error=f"Search failed: {e}. Please ensure Dash is running and the API server is enabled (Settings > Integration, or run open -b com.kapeli.dash-setapp, followed by defaults write com.kapeli.dash-setapp DHAPIServerEnabled YES).")
